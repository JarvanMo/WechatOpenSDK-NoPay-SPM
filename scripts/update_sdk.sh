#!/usr/bin/env bash
# 用法: ./scripts/update_sdk.sh 2.0.8
# 下载指定版本的微信 OpenSDK NoPay XCFramework，替换仓库中的 WechatOpenSDK-NoPay.xcframework，
# 重新生成 WechatOpenSDK-NoPay-Dynamic.xcframework，然后提交、打 tag 并推送。
set -euo pipefail
...
FRAMEWORK_NAME="WechatOpenSDK-NoPay.xcframework"
DYNAMIC_FRAMEWORK_NAME="WechatOpenSDK-NoPay-Dynamic.xcframework"
URL="https://dldir1.qq.com/WechatWebDev/opensdk/XCFramework/OpenSDK${VERSION}_NoPay.zip"
ZIP_FILE="$ROOT_DIR/OpenSDK${VERSION}_NoPay.zip"
WORK_DIR="$(mktemp -d)"
...
echo "==> 重新生成 $DYNAMIC_FRAMEWORK_NAME"
"$ROOT_DIR/scripts/build_dynamic_xcframework.sh"

echo "==> 提交"
git add -A "$FRAMEWORK_NAME" "$DYNAMIC_FRAMEWORK_NAME"
if git diff --cached --quiet; then
  echo "没有变化，无需提交（可能已是 $VERSION）"
  exit 0
fi
git commit -m "bump framework to $VERSION"
